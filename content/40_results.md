...
Subsection \ref{identifying-heavy-io} shows how fine-grained measurements enable us to break the total rate down into its components which we can use to identify users who perform heavy I/O.
...
Furthermore, we can visually determine a *threshold* between the average and heavy I/O from a density plot.
We should select a threshold such that the *average I/O*, with many small values, is below the threshold, and the *heavy I/O*, with a few large values, is above the threshold.
...
We can visually determine a threshold between average and heavy I/O at $10^1$ operations per second.
We can see two heavy I/O patterns compared to the average behavior; many intense spikes and three less intense bursts.
\label{fig:density-1}
...
We can visually determine a threshold between average and heavy I/O at $10^2$ operations per second.
...
We can visually determine a threshold between average and heavy I/O at $10^8$ bytes per second.
